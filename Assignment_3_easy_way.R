...
remove_these = paste0(list.files(pattern = "df"))
...
df1 = data.frame(Col1 = c(1:100), Col2 = c(rep("A",50), rep("B", 50)))
df2 = data.frame(Col1 = c(1:100), Col2 = c(rep("C",20), rep("D", 80)))
?left_join
?right_join
?inner_join
?full_join
df3 = full_join(df1, df2, by = "Col1")
left_join(df1,df2, by = "Col2")
View(df3)
rnorm(10)
...
list.files()
df = read.delim("US_Biome_Diversity.tsv", sep = "\t")
decreasing = order(df$Diversity, decreasing = TRUE)
df_ordered = df[decreasing,]
